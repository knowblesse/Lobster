...
for f = 1 : numel(workingfile)
    ...
    X = generateWholeSessionUnitdata(TANK_location, SessionTime_s, KERNEL_SIZE, KERNEL_STD, TIMEWINDOW_BIN, locPsec);
    ...
    if (sum(sum(isnan(X(REMOVE_START_SEC * locPsec : end - REMOVE_END_SEC * locPsec,:)))) ~= 0)
        error('nan is in the data!');
    end

    writematrix(X(REMOVE_START_SEC * locPsec+1: end - REMOVE_END_SEC * locPsec,:),X_data_path,'Delimiter',',');
end
