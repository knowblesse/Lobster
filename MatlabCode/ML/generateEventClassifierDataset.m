    ...
    for trial = 1 : numTrial
        ...
        if (LICK_window(1) >= 1) && (LICK_window(2) <= numel(whole_serial_data))
            ...
            LICK{trial} = mean(reshape(...
                (conv(serial_data(LICK_window(1)+1:LICK_window(2)),kernel,'same') - serial_data_mean) ./ serial_data_std,...
                TIMEWINDOW_BIN, binnedDataSize), 1);
        end

        if (IROF_window(1) >= 1) && (IROF_window(2) <= numel(whole_serial_data))
            IROF{trial} = mean(reshape(...
                (conv(serial_data(IROF_window(1)+1:IROF_window(2)),kernel,'same') - serial_data_mean) ./ serial_data_std,...
                TIMEWINDOW_BIN, binnedDataSize), 1);
        end
    end
