...
parser.add_argument('--doorClosedOnly', default='False', required=False)
...
args.doorClosedOnly = strinput2bool(args.doorClosedOnly)

def NeuralRegressor(tankPath, outputPath, dataset, device,
                    neural_data_rate, truncatedTime_s, train_epoch, init_lr, PFI_numRepeat, numBin,
                    removeNestingData, removeEncounterData, removeWanderData, stratifyData, doorClosedOnly):
    rng = default_rng()
    # Load Tank
...
for i, tank in enumerate(sorted([p for p in InputFolder.glob('#*')])):
    ...
    NeuralRegressor(
            ...
            doorClosedOnly=args.doorClosedOnly
            )
