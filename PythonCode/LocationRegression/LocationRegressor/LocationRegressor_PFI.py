...
def fitDNN(X, y, device, init_lr, train_epoch, verbose=True):
    # Run Test
    kf = KFold(n_splits=5, shuffle=True)
    WholeTestResult = np.zeros([X.shape[0], 3])  # num data x [real, fake, predicted]
    WholeTestResult[:, :1] = y
...
def DistanceRegressor(tankPath, outputPath, device, neural_data_rate, truncatedTime_s, train_epoch, init_lr):
    rng = default_rng()
    # Load Tank
    tank_name = re.search('#.*', str(tankPath))[0]
    print(tank_name)
    ...
    # Run Test
    WholeTestResult = fitDNN(X, y, device, init_lr, train_epoch)

    defaultLoss = np.mean(np.abs(WholeTestResult[:,0]-WholeTestResult[:,2]))
    print(defaultLoss)
    ...
    numRepeat = 10
    importanceScore = np.zeros((numRepeat, numUnit))
    for unit in range(numUnit):
        ...
        for rep in range(numRepeat):
            ...
            WholeTestResult_ = fitDNN(X_corrupted, y, device, init_lr, train_epoch, verbose=False)
            importanceScore[rep, unit] = np.mean(np.abs(WholeTestResult_[:,0]-WholeTestResult_[:,2])) - defaultLoss
            print(f'{rep} ', end='')
        print(f'unit : {unit} ')
    print(f"{tank_name} Complete")
    savemat(outputPath/f'{tank_name}result.mat', 
            {'WholeTestResult': WholeTestResult, 'importanceScore': importanceScore})
...
for i, tank in enumerate(sorted([p for p in InputFolder.glob('#*')])):
    print(f'{i:02} {tank}')
    DistanceRegressor(tank, OutputFolder, device, neural_data_rate=2, truncatedTime_s=10, train_epoch=20000, init_lr=0.0001)
