...
for tank in pbar:
    ...
    # remove neural data around +- 1 second from HE and HW
    critical_event_time = []
    for trial, ir, lick, attack in ParsedData:
        ...
        HE_time = ir[0, 0] + trial[0, 0]  # first IRON. not valid IRON
        HW_time = ir[nearAttackIRindex, 1]  + trial[0, 0] # valid IROF
        critical_event_time.append(HE_time)
        critical_event_time.append(HW_time)

    mask = np.zeros(midPointTimes.shape, dtype=bool)
    for et in critical_event_time:
        mask = np.logical_or(mask, np.logical_and(et - 1 <= midPointTimes, midPointTimes <= et + 1))

    neural_data_transformed_pca = pca.fit_transform(data['neural_data'][~mask, :])

    # Calculate Centroids
    centroids_pca = {
        'nest': np.mean(neural_data_transformed_pca[zoneClass[~mask] == 0, :],0),
        'foraging': np.mean(neural_data_transformed_pca[zoneClass[~mask] == 1, :],0),
        'encounter': np.mean(neural_data_transformed_pca[zoneClass[~mask] == 2, :],0)}
    ...
    # Compare within-zone distance vs between-zone distance
    NN = np.mean(np.sum((centroids_pca['nest'] - neural_data_transformed_pca[zoneClass[~mask] == 0, :]) ** 2, axis=1) ** 0.5)
    NO = np.mean(
        [np.mean(np.sum((centroids_pca['foraging'] - neural_data_transformed_pca[zoneClass[~mask] == 0, :]) ** 2, axis=1) ** 0.5),
         np.mean(np.sum((centroids_pca['encounter'] - neural_data_transformed_pca[zoneClass[~mask] == 0, :]) ** 2, axis=1) ** 0.5)]
    )

    FF = np.mean(np.sum((centroids_pca['foraging'] - neural_data_transformed_pca[zoneClass[~mask] == 1, :]) ** 2, axis=1) ** 0.5)
    FO = np.mean(
        [np.mean(np.sum((centroids_pca['nest'] - neural_data_transformed_pca[zoneClass[~mask] == 1, :]) ** 2, axis=1) ** 0.5),
         np.mean(np.sum((centroids_pca['encounter'] - neural_data_transformed_pca[zoneClass[~mask] == 1, :]) ** 2, axis=1) ** 0.5)]
    )
    F_std = np.mean(np.std(neural_data_transformed_pca[zoneClass[~mask] == 1, :], axis=0))

    EE = np.mean(np.sum((centroids_pca['encounter'] - neural_data_transformed_pca[zoneClass[~mask] == 2, :]) ** 2, axis=1) ** 0.5)
    EO = np.mean(
        [np.mean(np.sum((centroids_pca['nest'] - neural_data_transformed_pca[zoneClass[~mask] == 2, :]) ** 2, axis=1) ** 0.5),
         np.mean(np.sum((centroids_pca['foraging'] - neural_data_transformed_pca[zoneClass[~mask] == 2, :]) ** 2, axis=1) ** 0.5)]
    )
    array2append.extend([
        NN, NO,
        FF, FO,
